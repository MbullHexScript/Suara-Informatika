...
const statusMeta: Record<string,{label:string, cls:string, icon:React.ReactNode, desc:string}> = {
  ...
  diproses:{ label:"Diproses", cls:"bg-white/70 border border-black/[0.08] text-[#1a1c1c] backdrop-blur", icon:<Loader2 className="w-3.5 h-3.5" />, desc:"Sedang ditindaklanjuti."},
  selesai:{ label:"Selesai", cls:"bg-black text-white", icon:<CheckCircle2 className="w-3.5 h-3.5" />, desc:"Tindak lanjut selesai."},
  ditolak:{ label:"Ditolak", cls:"bg-white/70 border border-black/[0.08] text-[#525252] backdrop-blur", icon:<XCircle className="w-3.5 h-3.5" />, desc:"Tidak dapat diproses."},
}
      ...
      <div className="container-wide py-8 md:py-10">
        <div className="max-w-[640px] mx-auto space-y-6">
          <div className="text-center space-y-2">
            <h1 className="text-[28px] font-[800]" style={{ letterSpacing:"-0.02em" }}>Lacak Status</h1>
          ...
          <div className="glass-card rounded-[16px] p-5 md:p-6 space-y-4 will-change-transform">
            ...
            {data && m && (
              <div className="space-y-4 pt-2">
                ...
                <div>
                  <h2 className="text-[16px] font-bold leading-snug" style={{ letterSpacing:"-0.01em" }}>{data.title}</h2>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className={`rounded-[12px] p-3 border will-change-transform ${["diproses","selesai"].includes(data.status)?"bg-black text-white border-black":"bg-white/60 border-black/[0.06] opacity-60"}`}><p className="label-sm text-[10px] opacity-70">DIPROSES</p><p className="text-[12px] font-bold mt-1">Tindak lanjut</p></div>
                  <div className={`rounded-[12px] p-3 border will-change-transform ${data.status==="selesai"?"bg-black text-white border-black":"bg-white/60 border-black/[0.06] opacity-60"}`}><p className="label-sm text-[10px] opacity-70">SELESAI</p><p className="text-[12px] font-bold mt-1">Tuntas</p></div>
                </div>
