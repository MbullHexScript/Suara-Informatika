...
import { Upload, X, CheckCircle2, Copy, Shield, Image as ImgIcon, Loader2, ArrowRight, Clock, Lock } from "lucide-react"
          ...
          <div className="max-w-[640px] mx-auto">
            <motion.div initial={reduce?false:{y:8,opacity:0}} animate={{y:0,opacity:1}} transition={{type:"spring",bounce:0,duration:0.35}} className="glass-card rounded-[16px] p-8 md:p-10 text-center space-y-5 will-change-transform">
              ...
              <div className="space-y-2">
                <h1 className="text-[22px] font-bold" style={{ letterSpacing:"-0.02em" }}>Laporan terkirim — terima kasih</h1>
                <p className="text-[13px] leading-[1.6] text-[#525252]">Simpan nomor tiket ini ya. Ini satu-satunya cara kamu bisa cek kabarnya nanti, tanpa membuka identitasmu.</p>
              </div>
              ...
              <p className="text-[11px] text-[#525252] inline-flex items-center gap-1.5 justify-center"><Shield className="w-3.5 h-3.5" /> Anonim penuh — kami jaga rahasiamu</p>
            </motion.div>
          </div>
        </div>
      </div>
          ...
          <motion.div initial={reduce?false:{y:16,opacity:0}} animate={{y:0,opacity:1}} transition={{type:"spring",bounce:0,duration:0.5,delay:0.08}} className="glass-card rounded-[16px] p-5 md:p-7 will-change-transform">
            <form onSubmit={submit} className="space-y-7">
              ...
              <motion.div initial={reduce?false:{y:8,opacity:0}} animate={{y:0,opacity:1}} transition={{type:"spring",bounce:0,duration:0.35,delay:0.08}} className="space-y-3 will-change-transform">
                <div className="flex items-center gap-2 label-sm"><span className="h-px w-6 bg-[#1a1c1c]" /> 02. DITUJUKAN KE</div>
                <p className="text-[12px] text-[#7e7576] -mt-1">Agar diteruskan ke pihak yang paling tepat.</p>
              ...
              <motion.div initial={reduce?false:{y:8,opacity:0}} animate={{y:0,opacity:1}} transition={{type:"spring",bounce:0,duration:0.35,delay:0.12}} className="space-y-3 will-change-transform">
                <div className="flex items-center gap-2 label-sm"><span className="h-px w-6 bg-[#1a1c1c]" /> 03. CERITAMU</div>
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <label className="label-sm text-[#525252]">KATEGORI</label>
                    <div className="relative">
                      <select value={category} onChange={e=>setCategory(e.target.value)} className="w-full h-12 px-4 pr-10 rounded-[12px] bg-white/80 border border-black/[0.08] text-[14px] appearance-none focus:outline-none focus:border-[#0A84FF] focus:ring-2 focus:ring-[#0A84FF]/20 backdrop-blur">
                        <option value="">Pilih yang paling dekat...</option>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between"><label className="label-sm text-[#525252]">JUDUL RINGKAS</label><span className="text-[11px] text-[#7e7576]">{title.length} / 100</span></div>
                    <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Misal: AC ruang lab sering mati siang hari" maxLength={100} className="w-full h-12 px-4 rounded-[12px] bg-white/80 border border-black/[0.08] text-[14px] placeholder:text-[#7e7576] focus:outline-none focus:border-[#0A84FF] focus:ring-2 focus:ring-[#0A84FF]/20 backdrop-blur" />
                  </div>
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between"><label className="label-sm text-[#525252]">CERITA LENGKAP</label><span className="text-[11px] text-[#7e7576]">{desc.length} / 2000</span></div>
                    <textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Tulis apa adanya — kapan terjadi, di mana, seperti apa dampaknya. Tidak perlu sempurna, yang jujur lebih membantu." maxLength={2000} rows={5} className="w-full min-h-[140px] p-4 rounded-[12px] bg-white/80 border border-black/[0.08] text-[14px] leading-[1.6] placeholder:text-[#7e7576] focus:outline-none focus:border-[#0A84FF] focus:ring-2 focus:ring-[#0A84FF]/20 backdrop-blur" />
                    <p className="text-[11px] text-[#7e7576]">Tips: tuliskan kronologi singkat + harapanmu. Kami baca dengan hati.</p>
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div initial={reduce?false:{y:8,opacity:0}} animate={{y:0,opacity:1}} transition={{type:"spring",bounce:0,duration:0.35,delay:0.16}} className="space-y-3 will-change-transform">
                <div className="flex items-center gap-2 label-sm"><span className="h-px w-6 bg-[#1a1c1c]" /> 04. BUKTI FOTO <span className="text-[11px] font-normal normal-case tracking-normal text-[#7e7576]">(opsional, bikin lebih kuat)</span></div>
                <label
                  ...
                  <span className="text-[14px] font-semibold">Tarik foto ke sini</span>
                  <span className="text-[12px] text-[#525252]">Atau klik untuk memilih — JPG/PNG/WebP, maks 5MB, hingga 5 foto</span>
                ...
                <div className="flex items-center gap-2 glass-pill p-3">
                  <span className="w-8 h-8 rounded-full bg-white border border-black/[0.06] grid place-items-center shrink-0"><Shield className="w-4 h-4" /></span>
                  <p className="text-[11px] leading-[1.5] text-[#525252]"><b className="text-[#1a1c1c]">Privasimu aman</b> — tidak ada nama, NIM, atau kontak yang kami simpan.</p>
                </div>
              </motion.div>

              <div className="pt-3 border-t border-black/[0.06] flex flex-col gap-3">
                <button type="submit" disabled={loading} onPointerDown={(e)=>(e.currentTarget as HTMLElement).setPointerCapture?.(e.pointerId)} className="pressable inline-flex items-center justify-center gap-2 h-12 px-7 rounded-full bg-black text-white text-[13px] font-semibold tracking-[0.04em] uppercase disabled:opacity-40 will-change-transform w-full md:w-auto md:self-end">
                  {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Mengirim dengan aman...</> : <>Kirim dengan aman <ArrowRight className="w-4 h-4" /></>}
                </button>
                <p className="text-[11px] text-center md:text-right text-[#7e7576]">Dengan mengirim, kamu percaya kami untuk menindaklanjuti dengan bijak.</p>
              </div>
            </form>
          </motion.div>
        </div>
